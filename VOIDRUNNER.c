...
static struct {
    ...
    f32  energy, maxenergy, shield, maxshield, shield_cd;
...
/* shared generator pool: weapons/boost use ENERGY. Death is ONLY energy<=0. */
static f32 energy_frac(void){ return G.maxenergy>0 ? clampf(G.energy/G.maxenergy,0,1) : 0; }
static void energy_spend_soft(f32 a){ G.energy-=a; if(G.energy<8.0f) G.energy=8.0f; }
static int energy_try_spend(f32 a){ if(G.energy <= a+8.0f) return 0; G.energy-=a; return 1; }
static void player_energy_hit(f32 a){ G.energy-=a; G.shield_cd=3.0f; }
...
static void new_game(u32 master){
    m_set(&G,0,sizeof(G));
    gen_galaxy(master);
    G.maxenergy=100; G.energy=100; G.maxshield=60; G.shield=60;
...
static void update(f32 dt, const u8* ks){
    ...
    if(G.shield_cd<=0 && G.cabin_temp<0.92f && G.energy<G.maxenergy){
        G.energy += (G.extra_energy?6.0f:3.5f)*dt;   /* reactor recovery, paused after hits */
    ...
    int boost = ks[SC_LSHIFT] && thrusting && G.energy>13.0f;
    ...
    { f32 bd=999999.0f; for(int si=0;si<G.nstar;si++){ f32 d=vlen(vsub(G.star[si].pos,G.ppos))-G.star[si].r; if(d<bd) bd=d; }
      ...
      if(G.cabin_temp>=0.995f){
          ...
          if(G.shield>0){ G.shield-=burn*dt; if(G.shield<0){ player_energy_hit(-G.shield); G.shield=0; } else G.shield_cd=3.0f; }
          else player_energy_hit(burn*dt*1.15f);
        ...
        if(hostile){
            ...
            if(d<520 && e->fire_cd<=0){
                ...
                if(frand() < 0.55f - d/1600.0f){
                    f32 dmg = 6 + frand()*8;
                    if(G.shield>0){ f32 sd=dmg*shield_hit_scale(); G.shield-=sd; if(G.shield<0){ player_energy_hit((-G.shield)*1.35f + 2.0f); G.shield=0; } else G.shield_cd=3.0f; }
                    else player_energy_hit(dmg*1.75f + 2.0f);
                    sfx_play(SFX_DAMAGE);
                }
            }
        } else {
    ...
    /* ---- death ---- */
    if(G.energy<=0){ G.state=ST_DEAD; G.energy=0; sfx_play(SFX_KILL); p_SDL_SetRelativeMouseMode(0); return; }
...
static void draw_hud(void){
    ...
    text(34,H-118,1.3f,"ENERGY",0.56f,0.72f,0.74f); bar(132,H-130,160,11,G.energy/G.maxenergy,0.88f,0.34f,0.30f);
    ...
    if(G.energy<18.0f) text(W/2-textw("ENERGY LOW",1.7f)/2, H/2+80, 1.7f, "ENERGY LOW", 1.0f,0.52f,0.22f);
...
static f32 music_menu_sample(f32 t){
    const f32 beat=0.8333333f;
    int q=(int)f_floor(t/beat);
    f32 x=t-(f32)q*beat, e=1.0f-x/beat; e*=e;
    static const int deg[8]={0,2,3,1,0,5,4,2};
    f32 f=note_hz(deg[q&7],(q&4)?2:1);
    f32 bass=note_hz((q&2)?5:0,-1);
    return e*(f_sin(TAU*f*x)*0.075f + f_sin(TAU*f*2.0f*x)*0.020f) + f_sin(TAU*bass*t)*0.035f;
}
        ...
        switch(G.state){
            ...
            case ST_DEAD:
                if(edge(ks,SC_RETURN)){
                    G.energy=G.maxenergy; G.shield=G.maxshield; G.wanted=0; G.legal=0;
